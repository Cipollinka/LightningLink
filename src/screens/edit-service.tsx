...
>;
export const EditServiceScreen: FC<Props> = ({route}) => {
  const {serviceId} = route.params;
  const {userProfile, setUserProfile} = useUserStateProfile();

  const service = userProfile?.services.find(
  ...
  const [links, setLinks] = useState<Link[]>(service?.links || []);
  const [selectedColor, setColor] = useState<string>(
    service?.color || '#FFC400',
  );
  const [category, setCategory] = useState<ServiceCategory>(
    service?.category || ServiceCategory.Study,
  );
  const [photo, setPhoto] = useState<string | undefined>(service?.cover);
  ...
  const onSave = async () => {
    if (userProfile) {
      await setUserProfile({
        ...userProfile,
        services: userProfile.services.map(s => {
          if (s.id === serviceId) {
            return {
              ...s,
              title,
